...
export const mockLogin = async (username: string, password: string): Promise<string> => {
	const user = users.find((u) => u.username === username && u.password === password)
	if (user) return btoa(`${username}:${password}`)
	else throw new Error('Invalid username or password')
}

export const mockRegister = async (username: string, password: string): Promise<string> => {
